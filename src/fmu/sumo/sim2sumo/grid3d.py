...
def upload_restart(
    ...
    prop_names = get_restart_properties(restart_path, xtgeoegrid, config["sim2sumoconfig"][datafile])
...
def upload_simulation_runs(config, dispatcher):
    ...
    Args:
        config (dict): the fmu config file with metadata and sim2sumoconfig
