...
def test_export_init(xtgeogrid, scratch_files):
...
#     case = InitializeCase(config=config)
#     root = path.parents[1]
#     case_metadata_path = case.export(
#         rootfolder=path.parents[1],
#         casename="tests_sim2sumo",
#         caseuser="sim2sumo",
#         restart_from=None,
#         description="For testing sim2sumo",
#     )
...
# def check_sumo(case_uuid, correct):
...
def test_export_restart(xtgeogrid, scratch_files):
    real0, eight_datafile, config_path = scratch_files
    prefix = "UNRST"
    expected_exports = 12
...
#     case_uuid = register(real0, config_path)
#     print(case_uuid)


def test_tmpdir(scratch_files, case_uuid):
    print(scratch_files)
    print(case_uuid)
